...
class cVAE_Model_2(nn.Module):
    ...
    def __init__(self, p_dims, q_dims, q_dims_att, q_dims_adj, dropout=0.5):
        ...
        temp_p_dims = [self.p_dims[0] * 2] + self.p_dims[1:]
        ...
        # 求解real interest
        temp_q_dims_real = self.q_dims[:-1] + [self.q_dims[-1] * 2]
        self.q_real_layers = nn.ModuleList([nn.Linear(d_in, d_out) for
                                            d_in, d_out in zip(temp_q_dims_real[:-1], temp_q_dims_real[1:])])

        for i in range(len(self.q_real_layers)):
            self.q_real_layers[i].weight = Parameter(
                self.p_layers[len(self.q_real_layers) - i - 1].weight.transpose(0, 1).contiguous())


        self.drop = nn.Dropout(dropout)
        self.init_weights()

    def forward(self, input_x_huai, input_att, input_adj, input_x_all):
        ...
        final_dec = torch.cat((z, adj_z), dim=-1)
        ...
        real_mu, real_logvar = self.encode_real_pre(input_x_all)  # 等价于 同adj共享参数
    ...
    def encode_real_pre(self, input):
        h = input
        for i, layer in enumerate(self.q_real_layers):
            h = layer(h)
            if i != len(self.q_real_layers) - 1:
                h = F.tanh(h)
            else:
                mu = h[:, :self.q_dims[-1]]
