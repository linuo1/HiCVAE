...
dataset_name = 'movieline'  #   movieline   
...
# qian6个都是True，后三个是False 0.9
# data_train_file = "../../train_test_{}/train_test_VAE_{}_att_adj_{}_{}.pkl".format(dataset_name, dataset_name, adj_num, sim_select)
data_train_file = "data/train_test_VAE_{}_att_adj_{}_bav_final.pkl".format(dataset_name, adj_num)
...
with open(data_att_file, 'rb') as f:
    ...
    gender_count, age_count, occupation_count, zip_count = pickle.load(f)
print(gender_count, age_count, occupation_count, zip_count)  # 2 7 21 3439
config_single = {'dataset': dataset_name,
                 'dropout': 0.8,
                 'train_batch_size': 256,
                 'test_batch_size': 256,
                 # 'layer': [128*3, 128], #64,32   128*2, 128
                 #
                 # 'alpha': 1,
                 # 'beta': 1,
                 'lr': 0.001,

                 'num_feature': gender_count + age_count + occupation_count + zip_count,
                 'att_count': 4,
                 'index_all': [0, gender_count, gender_count + age_count,
                               gender_count + age_count + occupation_count],
                 'num_d': item_count + 1,
...
d = 128 ## [16,32,64,128,256]
# if dataset_name == "movieline":
p_dims = [d, d * 3, config_all['num_item']]
q_dims = [config_all['num_item'], d * 3, d]
q_dims_att = [config_single['num_feature'], d * 2, d]
...
for epoch in range(num_epoch):  # 第一次迭代的 all data
    ...
    f11 = open('model_{}_{}_{}_{}_{}_{}_{}_{}_final.txt'.format(model_name, dataset_name, load_model, para_dong, propa, adj_num, select_model, d), 'a+')
    ...
    if test_HR_old > best_HR:
        ...
        # path = 'model/{}_MVAE_all_{}_{}_{}_{}_{}'.format(dataset_name, load_model, para_dong, propa, adj_num, select_model)
        # for l in p_dims:
        #     path += '_' + str(l)
        # cVAE_model.cpu()
